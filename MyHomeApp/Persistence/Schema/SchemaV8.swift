...
/// VersionedSchema v8.0.0 — copies V7's models verbatim, adds npsSchemeCode to Asset,
/// and introduces three new @Models: SIP, SIPAmountChange, and Contribution (D-08, Phase 11.1).
///
/// Rules:
/// - SchemaV1.swift through SchemaV7.swift are IMMUTABLE after they ship. Never edit them.
/// - SchemaV8 is an additive superset: it copies SchemaV7's models verbatim and
...
enum SchemaV8: VersionedSchema {
    ...
    // MARK: - Category @Model (copied verbatim from SchemaV7 — no V8 changes)
    ...
    // MARK: - Expense @Model (copied verbatim from SchemaV7 — no V8 changes)
    ...
    final class Expense {
        ...
        @Relationship(deleteRule: .nullify, inverse: \SchemaV8.Category.expenses)
        var categories: [SchemaV8.Category] = []
    ...
    // MARK: - Note @Model (copied verbatim from SchemaV7 — no V8 changes)
    ...
    final class Note {
        ...
        @Relationship(deleteRule: .cascade, inverse: \SchemaV8.NoteBlock.note)
        var blocks: [SchemaV8.NoteBlock]? = []
    ...
    // MARK: - NoteBlock @Model (copied verbatim from SchemaV7 — no V8 changes)
    ...
    // MARK: - Account @Model (copied verbatim from SchemaV7 — no V8 changes)
    ...
    // MARK: - Asset @Model (additive V8 change: npsSchemeCode appended — D-01, D-08)
    ...
    final class Asset {
        ...
        // --- NEW in SchemaV8: NPS scheme code (D-01, D-08) ---
        // Append AFTER all V7 Asset fields (additive only — never reorder/remove existing fields).
        // nil for mutual funds, stocks, and NPS holdings not yet linked to a scheme.
    ...
    // MARK: - NetWorthSnapshot @Model (copied verbatim from SchemaV7 — no V8 changes)
    ...
    // MARK: - SIP @Model (NEW in SchemaV8 — D-04, D-08, Phase 11.1)
