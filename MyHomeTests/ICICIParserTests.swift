...
// Plan 07-01 — RED phase: ICICIParser + corpus fixtures are unimplemented (plan 05, corpus-gated).
// Tests FAIL RED via Issue.record until plan 05 provides ICICIParser and the .eml corpus.
...
struct ICICIParserTests {
    ...
    func canHandleRejectsOTP() {
        Issue.record("ICICIParser + fixtures unimplemented — plan 05 (corpus-gated)")
        // let sut = ICICIParser()
        // #expect(!sut.canHandle(sender: "alerts@icicibank.com", subject: "Your OTP is 123456"))
    }
    ...
    func canHandleRejectsPromo() {
        Issue.record("ICICIParser + fixtures unimplemented — plan 05 (corpus-gated)")
        // let sut = ICICIParser()
        // #expect(!sut.canHandle(sender: "alerts@icicibank.com", subject: "Special offer: free credit card"))
    }
    ...
    func canHandleRejectsVerification() {
        Issue.record("ICICIParser + fixtures unimplemented — plan 05 (corpus-gated)")
        // let sut = ICICIParser()
        // #expect(!sut.canHandle(sender: "alerts@icicibank.com", subject: "Verify your account"))
    }
    ...
    func canHandleRejectsStatement() {
        Issue.record("ICICIParser + fixtures unimplemented — plan 05 (corpus-gated)")
        // let sut = ICICIParser()
        // #expect(!sut.canHandle(sender: "alerts@icicibank.com", subject: "ICICI Bank Account Statement"))
    }
    ...
    func canHandleRejectsNonICICISender() {
        Issue.record("ICICIParser + fixtures unimplemented — plan 05 (corpus-gated)")
        // let sut = ICICIParser()
        // #expect(!sut.canHandle(sender: "alerts@hdfcbank.com", subject: "Transaction alert"))
    }
    ...
    @Test("parse: credit card spend fixture → expected amount + merchant — ING-06", arguments: [
        ("icici_cc_spend_1.eml", Decimal(2100), "Amazon", false),
        ("icici_upi_debit_1.eml", Decimal(350), "Ola", false),
        ("icici_refund_1.eml", Decimal(-800), "Swiggy", true),
    ])
