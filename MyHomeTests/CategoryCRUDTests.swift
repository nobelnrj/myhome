...
struct CategoryCRUDTests {
    ...
    /// STAB-03: a newly added custom category must surface at the TOP of the list
    /// (min(existing.sortOrder)-1), never the bottom. This locks in the insertion contract
    /// used by ManageCategoriesView.addCategory so it cannot silently regress.
    @Test("STAB-03: new category prepends at min(sortOrder)-1, not the bottom")
    func newCategoryPrependsAtTop() throws {
        let container = try makeContainer()
        let context = container.mainContext
        ...
        #expect(sorted.first?.name == "Custom", "STAB-03: new category must be first in the list")
        #expect(sorted.first?.sortOrder == -1, "STAB-03: new category sortOrder must be min-1 (-1)")
    }
}
